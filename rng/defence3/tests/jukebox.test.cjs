...
assert.equal(host.box.snapshot().playing, true);
host.advance(1000); client.advance(1000);
client.node('play').listeners.click();
assert.equal(host.box.snapshot().playing, false); assert.equal(client.context.player.playing, false);
const revision = client.box.snapshot().revision;
client.box.handle({peer:'imposter'}, {type:'JUKEBOX_STATE', state:{videoId:'9JD2nH4M8Dc',playing:true,position:0,revision:999}});
assert.equal(client.box.snapshot().revision, revision);
client.box.handle(fromHost, {type:'JUKEBOX_STATE', state:{videoId:'9JD2nH4M8Dc',playing:true,position:0,revision:0}});
assert.equal(client.box.snapshot().revision, revision);
host.node('play').listeners.click(); host.advance(5000); client.advance(5000); host.context.heartbeat();
assert.equal(client.box.snapshot().playing,true);
assert.equal(client.context.player.muted,true);
assert.ok(Math.abs(host.box.snapshot().position-client.box.snapshot().position)<.01);
...
console.log('PASS: shared links, pause/resume, local mute, late/rejoining peers, host authority, stale packets, drift correction, URL validation and autoplay recovery.');
